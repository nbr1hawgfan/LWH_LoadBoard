...
function initMap(){if(state.map)return;state.map=L.map("shipmentMap",{zoomControl:true}).setView([37.8,-96],4);L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{maxZoom:18,attribution:"&copy; OpenStreetMap contributors"}).addTo(state.map);state.mapLayer=L.layerGroup().addTo(state.map);setTimeout(()=>{state.map.invalidateSize();renderMap()},100)}
