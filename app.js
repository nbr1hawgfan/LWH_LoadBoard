...
const state={loadRows:[],loads:[],inventory:[],labor:[],loadRange:"today",laborRange:"today",inventoryView:"cards",calendarMonth:new Date(new Date().getFullYear(),new Date().getMonth(),1),map:null,mapLayer:null,mapMarkers:[],cityCoordinates:new Map(),settings:{...DEFAULT_SETTINGS},deferredInstall:null};
...
function normalizeInventory(raw){
  const receivedRaw=pick(raw,"DateReceived","ReceivedDate","SystemCreatedOn","CreatedOn","Received","InboundDate");
  const explicitAgeRaw=pick(raw,"AgeDays","DaysInInventory","DaysOld");
  const received=parseDate(receivedRaw);
  const hasExplicitAge=clean(explicitAgeRaw)!=="";
  const ageKnown=hasExplicitAge||!!received;
  const age=hasExplicitAge?num(explicitAgeRaw):(received?Math.max(0,Math.floor((new Date()-received)/86400000)):null);
  return{lwh:clean(pick(raw,"LWH_ID","LWHID","ControlNumber","PalletID")),customer:clean(pick(raw,"Customer","SubCustNm","SubCust","BillToName")),customerId:clean(pick(raw,"Customer_ID","CustomerID","Comments")),item:clean(pick(raw,"ItemNm","Item","ItemNumber")),description:clean(pick(raw,"ItemDescription","ItemDesc","Description1")),lot:clean(pick(raw,"LotNum","Lot","LotNm")),qty:num(pick(raw,"Qty","Quantity")),units:num(pick(raw,"Units","UnitCount"))||1,bay:clean(pick(raw,"BayName","Bay","Location")),warehouse:clean(pick(raw,"Warehouse","WHSE","LocationCode","Location")),received,age,ageKnown,invReceipt:clean(pick(raw,"INV_Receipt","InvReceipt")),raw}
}
...
async function refreshAll(){
  ...
  if(inventoryResult.status==="fulfilled"){
    state.inventory=parseCSV(inventoryResult.value.text).map(normalizeInventory).filter(x=>x.lwh||x.item||x.customer);
...
function filteredInventory(){const q=upper($("globalSearch").value),w=$("globalWarehouse").value,c=$("inventoryCustomer").value,item=upper($("inventoryItem").value),bay=upper($("inventoryBay").value),age=+$("inventoryAge").value;return state.inventory.filter(x=>{if(w&&x.warehouse!==w)return false;if(c&&x.customer!==c)return false;if(item&&!upper(x.item).includes(item))return false;if(bay&&!upper(x.bay).includes(bay))return false;if(age&&(!x.ageKnown||x.age<age))return false;if(q&&!upper([x.lwh,x.customer,x.customerId,x.item,x.description,x.lot,x.bay,x.warehouse,x.invReceipt].join(" ")).includes(q))return false;return true})}
...
function renderInventory(){const rows=filteredInventory();const ageAvailable=rows.some(x=>x.ageKnown);$("inventoryAge").disabled=!state.inventory.some(x=>x.ageKnown);$("inventoryAge").title=state.inventory.some(x=>x.ageKnown)?"":"The inventory CSV does not include a received date or age column.";$("inventoryKpis").innerHTML=[kpi("Records",fmtNum(rows.length)),kpi("Units",fmtNum(rows.reduce((s,x)=>s+x.units,0))),kpi("Quantity",fmtNum(rows.reduce((s,x)=>s+x.qty,0))),kpi("Customers",new Set(rows.map(x=>x.customer).filter(Boolean)).size),kpi("90+ Days",ageAvailable?rows.filter(x=>x.ageKnown&&x.age>=90).length:"Unavailable")].join("");const host=$("inventoryResults");if(state.inventoryView==="table"){host.innerHTML=`<div class="table-wrap"><table><thead><tr><th>LWH ID</th><th>Warehouse</th><th>Customer</th><th>Item</th><th>Description</th><th>Lot</th><th>Bay</th><th>Units</th><th>Qty</th><th>Age</th></tr></thead><tbody>${rows.map(x=>`<tr><td>${esc(x.lwh)}</td><td>${esc(x.warehouse)}</td><td>${esc(x.customer)}</td><td>${esc(x.item)}</td><td>${esc(x.description)}</td><td>${esc(x.lot)}</td><td>${esc(x.bay)}</td><td>${fmtNum(x.units)}</td><td>${fmtNum(x.qty)}</td><td>${x.ageKnown?fmtNum(x.age):"—"}</td></tr>`).join("")}</tbody></table></div>`}else{host.innerHTML=`<div class="inventory-grid">${rows.slice(0,1000).map(x=>`<article class="card inventory-card"><h3>${esc(x.item||"Item not listed")}</h3><p class="muted">${esc(x.description)}</p><div class="inventory-detail"><div><span>LWH ID</span><b>${esc(x.lwh||"—")}</b></div><div><span>Warehouse</span><b>${esc(x.warehouse||"—")}</b></div><div><span>Customer</span><b>${esc(x.customer||"—")}</b></div><div><span>Bay</span><b>${esc(x.bay||"—")}</b></div><div><span>Lot</span><b>${esc(x.lot||"—")}</b></div><div><span>Age</span><b>${x.ageKnown?fmtNum(x.age)+" days":"Not provided"}</b></div><div><span>Units</span><b>${fmtNum(x.units)}</b></div><div><span>Quantity</span><b>${fmtNum(x.qty)}</b></div></div></article>`).join("")}</div>${rows.length>1000?'<p class="muted">Showing the first 1,000 matching cards. Use Table view for the full result.</p>':""}`}}
...
function renderAnalytics(){const loads=state.loads.filter(commonLoadFilter);$("analyticsWarehouse").innerHTML=renderBars(groupStats(loads,x=>x.whse||"Not listed").slice(0,15),"loads");$("analyticsDirection").innerHTML=renderBars(groupStats(loads,x=>x.direction||"Unknown"),"loads");$("analyticsCustomers").innerHTML=renderBars(groupStats(loads,x=>x.customer||"Not listed").slice(0,15),"loads");const m=new Map();loads.forEach(l=>l.items.forEach(i=>{const v=m.get(i.item)||{name:i.item,loads:0,units:0,qty:0};v.loads++;v.units+=i.units;v.qty+=i.qty;m.set(i.item,v)}));$("analyticsItems").innerHTML=renderBars([...m.values()].sort((a,b)=>b.loads-a.loads).slice(0,15),"loads")}
function initMap(){
  if(state.map)return;
  if(typeof L==="undefined"){showDialog('<div class="dialog-content"><h2>Map library unavailable</h2><p>Leaflet JavaScript did not load. Refresh the page after checking the internet connection.</p></div>');return}
  state.map=L.map("shipmentMap",{zoomControl:true,preferCanvas:true}).setView([37.8,-96],4);
  L.tileLayer("https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png",{maxZoom:18,tileSize:256,zoomOffset:0,attribution:"&copy; OpenStreetMap contributors"}).addTo(state.map);
  state.mapLayer=L.layerGroup().addTo(state.map);
  requestAnimationFrame(()=>requestAnimationFrame(()=>{state.map.invalidateSize(true);renderMap()}));
  setTimeout(()=>state.map&&state.map.invalidateSize(true),500)
}
async function geocodeCity(key){if(state.cityCoordinates.has(key))return state.cityCoordinates.get(key);const cache=JSON.parse(localStorage.getItem("lwh-city-coordinates-v2")||"{}");if(cache[key]){state.cityCoordinates.set(key,cache[key]);return cache[key]}const [city,stateCode]=key.split("|");try{const url=`https://geocoding-api.open-meteo.com/v1/search?name=${encodeURIComponent(city+", "+stateCode)}&count=10&language=en&format=json&countryCode=US`;const r=await fetch(url);const d=await r.json();const best=(d.results||[]).find(x=>upper(x.admin1).includes(stateCode)||upper(x.name)===city)||(d.results||[])[0];if(!best)return null;const coord={lat:best.latitude,lon:best.longitude};cache[key]=coord;localStorage.setItem("lwh-city-coordinates-v2",JSON.stringify(cache));state.cityCoordinates.set(key,coord);return coord}catch(e){console.error(e);return null}}
async function renderMap(){if(!state.map)return;state.mapLayer.clearLayers();const cities=groupStats(state.loads.filter(commonLoadFilter),x=>x.mapKey||"").filter(x=>x.name);$("mapCityList").innerHTML=cities.map(c=>`<button class="city-list-button" data-city="${esc(c.name)}"><b>${esc(c.name.replace("|",", "))}</b><span>${c.loads} loads · ${fmtNum(c.units)} units</span></button>`).join("")||'<div class="empty">No mapped cities.</div>';$("mapCityList").querySelectorAll("[data-city]").forEach(b=>b.onclick=()=>focusCity(b.dataset.city));const bounds=[];for(const c of cities.slice(0,150)){const p=await geocodeCity(c.name);if(!p)continue;bounds.push([p.lat,p.lon]);const marker=L.circleMarker([p.lat,p.lon],{radius:Math.min(18,6+Math.sqrt(c.loads)*2),color:getComputedStyle(document.documentElement).getPropertyValue("--accent").trim(),weight:3,fillColor:"#fff",fillOpacity:.9}).addTo(state.mapLayer);marker.bindPopup(`<b>${esc(c.name.replace("|",", "))}</b><br>${c.loads} loads<br>${fmtNum(c.units)} units<br><button onclick="window.filterMapCity('${esc(c.name)}')">View loads</button>`)}state.mapMarkers=bounds;if(bounds.length&&state.map.getZoom()<=4)state.map.fitBounds(bounds,{padding:[30,30],maxZoom:7})}
async function focusCity(key){const p=await geocodeCity(key);if(p)state.map.setView([p.lat,p.lon],8)}
window.filterMapCity=key=>{const [city,stateCode]=key.split("|");$("globalSearch").value=`${city} ${stateCode}`;navigate("loads");renderAll()}
...
function navigate(name){document.querySelectorAll(".module").forEach(m=>m.classList.toggle("active",m.id===name+"Module"));document.querySelectorAll(".nav-item").forEach(n=>n.classList.toggle("active",n.dataset.module===name));$("sidebar").classList.remove("open");if(name==="map"){initMap();setTimeout(()=>state.map.invalidateSize(),100)}localStorage.setItem("lwh-last-module",name)}
...
["inventoryCustomer","inventoryItem","inventoryBay","inventoryAge"].forEach(id=>$(id).addEventListener("input",renderInventory));document.querySelectorAll("[data-inv-view]").forEach(b=>b.onclick=()=>{state.inventoryView=b.dataset.invView;document.querySelectorAll("[data-inv-view]").forEach(x=>x.classList.toggle("active",x===b));renderInventory()});
$("fitMapBtn").onclick=()=>{if(state.mapMarkers.length)state.map.fitBounds(state.mapMarkers,{padding:[30,30],maxZoom:7})};
