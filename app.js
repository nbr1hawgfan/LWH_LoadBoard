...
const state={loadRows:[],loads:[],inventory:[],labor:[],loadRange:"today",laborRange:"today",mapRange:"today",inventoryView:"cards",calendarMonth:new Date(new Date().getFullYear(),new Date().getMonth(),1),mapVisited:false,cityCoordinates:new Map(),settings:{...DEFAULT_SETTINGS},deferredInstall:null};
...
function renderAll(){renderDashboard();renderLoads();renderCalendar();renderInventory();renderLabor();renderAnalytics();if(state.mapVisited)renderMap()}
...
/* ---------- Shipment map: flat SVG projection, no external tiles ---------- */
const MAP={lonMin:-125,lonMax:-66,latMin:24,latMax:50,latCos:Math.cos(37*Math.PI/180),scale:18.5,margin:10};
function project(lon,lat){const x=MAP.margin+(lon-MAP.lonMin)*MAP.latCos*MAP.scale;const y=MAP.margin+(MAP.latMax-lat)*MAP.scale;return[x,y]}
function inBounds(lon,lat){return lon>=MAP.lonMin-3&&lon<=MAP.lonMax+3&&lat>=MAP.latMin-2&&lat<=MAP.latMax+2}
function ringPath(ring){return"M"+ring.map(pt=>project(pt[0],pt[1]).map(n=>n.toFixed(1)).join(",")).join("L")+"Z"}
function stateFeaturePath(f){if(f.type==="Polygon")return f.coords.map(ringPath).join(" ");return f.coords.map(poly=>poly.map(ringPath).join(" ")).join(" ")}
let STATES_SVG="";
function statesLayer(){if(typeof US_STATES==="undefined"||!US_STATES?.features)return"";if(STATES_SVG)return STATES_SVG;STATES_SVG=US_STATES.features.map(f=>`<path class="state" d="${stateFeaturePath(f)}"/>`).join("");return STATES_SVG}
...
async function renderMap(){
  ...
  const cities=groupStats(filteredLoads(state.mapRange),x=>x.mapKey||"").filter(x=>x.name);
  $("mapCityList").innerHTML=cities.map(c=>`<button class="city-list-button" data-city="${esc(c.name)}"><b>${esc(c.name.replace("|",", "))}</b><span>${c.loads} loads · ${fmtNum(c.units)} units</span></button>`).join("")||'<div class="empty">No mapped cities for this range.</div>';
  $("mapCityList").querySelectorAll("[data-city]").forEach(b=>b.onclick=()=>pulseCity(b.dataset.city));
  if(typeof US_STATES==="undefined"){
    $("shipmentMap").innerHTML='<div class="empty">Map outline failed to load. Refresh the page — if this keeps happening, confirm <code>us-map-data.js</code> deployed alongside the other files.</div>';
    return;
  }
  const maxLoads=Math.max(1,...cities.map(c=>c.loads));
  const points=[];
  for(const c of cities.slice(0,300)){
    let p;
    try{p=await geocodeCity(c.name)}catch(e){console.error(e);continue}
    if(!p||!inBounds(p.lon,p.lat))continue;
    const [x,y]=project(p.lon,p.lat);
    const r=(3+Math.sqrt(c.loads/maxLoads)*14).toFixed(1);
    points.push(`<circle class="city-dot" data-city="${esc(c.name)}" cx="${x.toFixed(1)}" cy="${y.toFixed(1)}" r="${r}"><title>${esc(c.name.replace("|",", "))} — ${c.loads} loads, ${fmtNum(c.units)} units</title></circle>`);
  }
  $("shipmentMap").innerHTML=`<svg viewBox="0 0 900 505" preserveAspectRatio="xMidYMid meet" role="img" aria-label="Shipment volume by city">${statesLayer()}<g>${points.join("")}</g></svg>`;
  $("shipmentMap").querySelectorAll("[data-city]").forEach(el=>el.addEventListener("click",()=>window.filterMapCity(el.dataset.city)));
}
function pulseCity(key){const el=$("shipmentMap").querySelector(`circle[data-city="${CSS.escape(key)}"]`);if(!el)return;el.classList.remove("pulse");void el.offsetWidth;el.classList.add("pulse");el.scrollIntoView({block:"center",behavior:"smooth"})}
...
document.querySelectorAll("[data-map-range]").forEach(b=>b.onclick=()=>{state.mapRange=b.dataset.mapRange;document.querySelectorAll("[data-map-range]").forEach(x=>x.classList.toggle("active",x===b));renderMap()});
