...
function makeLoadKey(r){const p=clean(pick(r.raw,"LoadKey"));return p||[dateKey(r.date),r.whse,r.direction,r.pro,r.billRef,r.relatedPro,r.customer].join("|")}
function groupLoads(rows){const map=new Map();rows.forEach(r=>{const key=makeLoadKey(r);if(!map.has(key))map.set(key,{key,date:r.date,whse:r.whse,direction:r.direction,customer:r.customer,pro:r.pro,relatedPro:r.relatedPro,billRef:r.billRef,carrier:r.carrier,shipper:r.shipper,conName:r.conName,status:r.status,instructions:r.instructions,comments:r.comments,items:[]});const load=map.get(key);if(r.item||r.units||r.qty){const itemKey=`${r.item}|${r.description}`,existing=load.items.find(i=>i.key===itemKey);if(existing){existing.units+=r.units;existing.qty+=r.qty}else load.items.push({key:itemKey,item:r.item||"Unspecified item",description:r.description,units:r.units,qty:r.qty})}});return[...map.values()].sort((a,b)=>(a.date?.valueOf()||0)-(b.date?.valueOf()||0))}
function loadMetrics(load){return{itemCount:load.items.length,units:load.items.reduce((s,i)=>s+i.units,0),qty:load.items.reduce((s,i)=>s+i.qty,0)}}
async function loadData(){ $("syncStatus").textContent="Refreshing published schedule…";$("refreshBtn").disabled=true;try{const joiner=CSV_URL.includes("?")?"&":"?";const response=await fetch(CSV_URL+joiner+"cacheBust="+Date.now(),{cache:"no-store"});if(!response.ok)throw new Error(`CSV request failed (${response.status})`);const text=await response.text();if(!text.trim())throw new Error("The published CSV was empty.");localStorage.setItem(CACHE_KEY,JSON.stringify({text,savedAt:new Date().toISOString()}));useCSV(text);$("syncStatus").textContent=`Live data refreshed ${new Date().toLocaleTimeString([],{hour:"numeric",minute:"2-digit"})}`;$("notice").hidden=true}catch(error){const cached=JSON.parse(localStorage.getItem(CACHE_KEY)||"null");if(cached?.text){useCSV(cached.text);$("syncStatus").textContent=`Offline copy from ${new Date(cached.savedAt).toLocaleString()}`;showNotice("The published Google Sheet could not be reached. Showing the last successfully downloaded copy.")}else{state.rows=[];state.loads=[];applyFilters();$("syncStatus").textContent="Unable to load published schedule";showNotice("The app could not read the published CSV. Confirm the Google Sheet remains published to the web as CSV.")}console.error(error)}finally{$("refreshBtn").disabled=false}}
function useCSV(text){const raw=parseCSV(text);state.rows=raw.map(normalize).filter(r=>r.date||r.pro||r.billRef||r.customer);state.loads=groupLoads(state.rows);populateFilters();applyFilters()}
