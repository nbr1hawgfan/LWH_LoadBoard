...
const DEFAULT_SETTINGS={theme:"executive",appearance:"dark",accent:"#6c63ff",density:"comfortable",textSize:"normal",defaultModule:"dashboard",warehouse:"",weather:true,weatherLocation:"fort-smith",seconds:true};
const INVENTORY_KNOWN_HEADERS=new Set(["LWH_ID","LWHID","CONTROLNUMBER","PALLETID","CUSTOMER","SUBCUSTNM","SUBCUST","BILLTONAME","CUSTOMER_ID","CUSTOMERID","ITEMNM","ITEM","ITEMNUMBER","ITEMDESCRIPTION","ITEMDESC","DESCRIPTION1","LOTNUM","LOT","LOTNM","QTY","QUANTITY","UNITS","UNITCOUNT","BAYNAME","BAY","LOCATION","WAREHOUSE","WHSE","LOCATIONCODE","DATERECEIVED","RECEIVEDDATE","SYSTEMCREATEDON","CREATEDON","RECEIVED","INBOUNDDATE","AGEDAYS","DAYSININVENTORY","DAYSOLD","INV_RECEIPT","INVRECEIPT","VENDOR","COMMENTS","UNIQUE2","UNIQUE3"]);
...
function applySettings(){document.body.className="";if(state.settings.theme!=="executive")document.body.classList.add("theme-"+state.settings.theme);if(state.settings.appearance==="light")document.body.classList.add("appearance-light");if(state.settings.density==="compact")document.body.classList.add("density-compact");if(state.settings.textSize!=="normal")document.body.classList.add("text-"+state.settings.textSize);document.documentElement.style.setProperty("--accent",state.settings.accent);document.documentElement.style.setProperty("--accent-soft",hexToSoft(state.settings.accent));$("weatherPanel").style.display=state.settings.weather?"grid":"none";$("globalWarehouse").value=state.settings.warehouse||"";const at=$("appearanceToggle");if(at)at.textContent=state.settings.appearance==="light"?"Dark":"Light"}
...
function fillSettings(){$("settingTheme").value=state.settings.theme;$("settingAppearance").value=state.settings.appearance;$("settingAccent").value=state.settings.accent;$("settingDensity").value=state.settings.density;$("settingTextSize").value=state.settings.textSize;$("settingDefaultModule").value=state.settings.defaultModule;$("settingWeather").checked=state.settings.weather;$("settingWeatherLocation").value=state.settings.weatherLocation;$("settingSeconds").checked=state.settings.seconds}
...
function filteredLoads(range=state.loadRange){const today=new Date();today.setHours(0,0,0,0);return state.loads.filter(commonLoadFilter).filter(l=>{if($("globalDate").value||range==="all")return true;const d=l.date;if(!d)return false;if(range==="today")return dateKey(d)===dateKey(today);if(range==="yesterday"){const y=new Date(today.getFullYear(),today.getMonth(),today.getDate()-1);return dateKey(d)===dateKey(y)}if(range==="week"){const s=startOfWorkweek(today),e=new Date(s.getFullYear(),s.getMonth(),s.getDate()+7);return d>=s&&d<e}const days=+range;return d>=today&&d<new Date(today.getFullYear(),today.getMonth(),today.getDate()+days)})}
...
const settingMap={settingTheme:"theme",settingAppearance:"appearance",settingAccent:"accent",settingDensity:"density",settingTextSize:"textSize",settingDefaultModule:"defaultModule",settingWarehouse:"warehouse",settingWeather:"weather",settingWeatherLocation:"weatherLocation",settingSeconds:"seconds"};
