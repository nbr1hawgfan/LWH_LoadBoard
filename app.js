...
const state={loadRows:[],loads:[],inventory:[],labor:[],loadRange:"today",laborRange:"today",inventoryView:"cards",calendarMonth:new Date(new Date().getFullYear(),new Date().getMonth(),1),mapVisited:false,cityCoordinates:new Map(),settings:{...DEFAULT_SETTINGS},deferredInstall:null};
...
let STATES_SVG="";
function statesLayer(){if(STATES_SVG)return STATES_SVG;STATES_SVG=US_STATES.features.map(f=>`<path class="state" d="${stateFeaturePath(f)}"/>`).join("");return STATES_SVG}
...
async function renderMap(){
  ...
  const cities=groupStats(state.loads.filter(commonLoadFilter),x=>x.mapKey||"").filter(x=>x.name);
  $("mapCityList").innerHTML=cities.map(c=>`<button class="city-list-button" data-city="${esc(c.name)}"><b>${esc(c.name.replace("|",", "))}</b><span>${c.loads} loads · ${fmtNum(c.units)} units</span></button>`).join("")||'<div class="empty">No mapped cities.</div>';
  ...
  for(const c of cities.slice(0,300)){
    const p=await geocodeCity(c.name);
